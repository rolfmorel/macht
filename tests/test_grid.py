...
def test_spawn_tile():
    ...
    pytest.raises(Exception, new_grid, 4, 4, 17)

    g = grid.Grid()
    g.spawn_tile(row=0, column=0)
    assert g[0][0]

    pytest.raises(Exception, g.spawn_tile, row=0, column=0)

    g.spawn_tile(row=0)
    g.spawn_tile(row=0)
    g.spawn_tile(row=0)
    assert all(g[0])
    pytest.raises(Exception, g.spawn_tile, row=0)

    g.spawn_tile(column=0)
    g.spawn_tile(column=0)
    g.spawn_tile(column=0)
    assert all(g[row_idx][0] for row_idx in range(4))
    pytest.raises(Exception, g.spawn_tile, column=0)
